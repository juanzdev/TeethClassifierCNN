...
class frontalizer():
    ...
    def frontalization(self,img_,facebb,p2d_):
        ...
        symfront_tr=np.round(frontal_sym).astype(np.uint8)
        frontal_bgr = cv2.cvtColor(symfront_tr, cv2.COLOR_RGB2BGR)
        return rawfrontal, frontal_bgr


if __name__ == "__main__":
    ##to make sure you have dlib 
    PATH_face_model = '../lib/shape_predictor_68_face_landmarks.dat'
    md_face = dlib.shape_predictor(PATH_face_model)
    face_det = dlib.get_frontal_face_detector()
    fronter = frontalizer('../lib/ref3d.pkl')
    #________________model initialisation 
    #

    # test your image 
    img_name = '../test2.jpg'#names[k]
    ...
    print symfront.shape
    crop_imgcrop_img = symfront[160:230, 110:210]
    cv2.imwrite("../img/output_test_img/mouthdetectsingle_crop_region_warp.jpg",crop_imgcrop_img)
    crop_img_resized = cv2.resize(crop_imgcrop_img, (100, 100), interpolation = cv2.INTER_CUBIC)
